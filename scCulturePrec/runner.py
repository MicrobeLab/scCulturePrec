...
from scCulturePrec.wrapper import wrap_train_subfiles, wrap_generate_feature_map, wrap_matcher, elastic_net, elastic_net_pred, shap_analysis
...
def main():
    ...
    # Subparser for shap-analysis
    parser_four = subparsers.add_parser('shap-analysis', help='SHAP analysis for feature importance')
    parser_four.add_argument('--bg', type=str, help='Background samples (.npy)')
    parser_four.add_argument('--train', type=str, help='Training samples (.npy)')
    parser_four.add_argument('--test', type=str, help='Test samples (.npy)')
    parser_four.add_argument('--weight', type=str, help='/path/to/weights.pth')
    parser_four.add_argument('--out', type=str, help='Output prefix')


    args = parser.parse_args()
    
    # Call the appropriate function based on the command
    sub_commands = ['create-pair', 'elastic-net', 'dl-model', 'shap-analysis']
    assert args.command in sub_commands, f"'{args.command}' should be one of: create-pair, elastic-net, dl-model, shap-analysis"
    if args.command == 'create-pair':
        ...
        model = SiameseHybrid(num_morphol=args.num_morphol, 
                embed_size_morphol=args.embed_size_morphol, embed_size_spectra=args.embed_size_spectra, 
                dim_before_morphol=200, dim_before_spectra=7600).to(device)
        shap_analysis(bg=args.bg, train=args.train, test=args.test, model=model, weight=args.weight, out_prefix=args.out)
